...
class App(GenerateMesh):
    ...
    def create_geometry(self):
        ...
        p6 = gmsh.model.geo.add_point(x1, y1+20.0e+3, 0.0)

        #Squared Intrusion points for sill
        p7 = gmsh.model.geo.add_point(1000, y1+20.0e+3, 0.0)
        p8 = gmsh.model.geo.add_point(-1000, y1+20.0e+3, 0.0)
        p9 = gmsh.model.geo.add_point(1000, -48000, 0.0)
        p10 = gmsh.model.geo.add_point(-1000,-48000, 0.0)
        ...
        self.c_mid1 = gmsh.model.geo.add_line(p5, p7)
        self.c_mid3 = gmsh.model.geo.add_line(p8, p6)
        self.c_xneg2 = gmsh.model.geo.add_line(p6, p1)

        #New connecting lines for crust
        self.c_xpos2 = gmsh.model.geo.add_line(p5, p3)
        self.c_ypos = gmsh.model.geo.add_line(p3, p4)
        self.c_xneg1 = gmsh.model.geo.add_line(p4, p6)

        #New connecting lines for intrusion
        self.c_int_r = gmsh.model.geo.add_line(p7, p9)
        self.c_int_bottom = gmsh.model.geo.add_line(p9, p10)
        self.c_int_l = gmsh.model.geo.add_line(p10, p8)
        self.c_int_top = gmsh.model.geo.add_line(p8, p7)
        ...
        c0 = gmsh.model.geo.add_curve_loop([self.c_yneg, self.c_xpos1, 
                                            self.c_mid1, self.c_int_r, self.c_int_bottom, self.c_int_l,
                                            self.c_mid3, self.c_xneg2])
        self.s_mantle = gmsh.model.geo.add_plane_surface([c0])

        #Crust
        c1 = gmsh.model.geo.add_curve_loop([-self.c_mid3, self.c_int_top, -self.c_mid1,
                                             self.c_xpos2, self.c_ypos, self.c_xneg1])
        self.s_crust = gmsh.model.geo.add_plane_surface([c1])

        #Intrusion
        #c2 = gmsh.model.geo.add_curve_loop([self.c_int_top, self.c_int_r, self.c_int_bottom, self.c_int_l])
        #self.s_intrusion = gmsh.model.geo.add_plane_surface([c2])


        c2 = gmsh.model.geo.add_curve_loop([-self.c_int_top, -self.c_int_l, -self.c_int_bottom, -self.c_int_r,])
        self.s_intrusion = gmsh.model.geo.add_plane_surface([c2])
    ...
    def mark(self):
        ...
        vertex_groups = (
            ...
            VertexGroup(name="intrusion", tag=15, dim=1, entities=[self.c_int_top, self.c_int_r, self.c_int_bottom, self.c_int_l])
        )
    ...
    def generate_mesh(self, cell):
        ...
        gmsh.model.mesh.field.setNumbers(field_distance, "CurvesList", [self.c_int_top, self.c_int_r, self.c_int_bottom, self.c_int_l])
